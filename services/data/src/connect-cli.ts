...
async function fetchManagedAuthApp(
  env: Pick<DataEnv, 'AUTH_ORIGIN' | 'AUTH_SERVICE'>,
  clientId: string,
  token: string,
  fetcher?: HttpFetcher,
): Promise<Response> {
  const url = `${env.AUTH_ORIGIN.replace(/\/$/, '')}/connect/cli/apps/${encodeURIComponent(clientId)}`;
  const init: RequestInit = { headers: { Authorization: `Bearer ${token}` } };

  // Tests may inject an HTTP fetcher. Production DATA -> AUTH traffic must use
  // the Cloudflare Service Binding instead of public workers.dev fetch().
  if (fetcher) return fetcher(url, init);
  return env.AUTH_SERVICE.fetch(new Request(url, init));
}

export async function verifyManagedAuthApp(
  request: Request,
  env: Pick<DataEnv, 'AUTH_ORIGIN' | 'AUTH_SERVICE'>,
  clientId: string,
  fetcher?: HttpFetcher,
): Promise<ManagedAuthApp | Response> {
  const token = bearer(request);
  if (!token) return jsonError(401, 'UNAUTHORIZED', 'Connect CLI token이 필요합니다.');
  let response: Response;
  try {
    response = await fetchManagedAuthApp(env, clientId, token, fetcher);
  } catch {
...
export async function handleConnectCliGetScopes(clientId: string, request: Request, env: DataEnv, fetcher?: HttpFetcher): Promise<Response> {
...
export async function handleConnectCliPutScopes(clientId: string, request: Request, env: DataEnv, fetcher?: HttpFetcher): Promise<Response> {
  const managed = await verifyManagedAuthApp(request, env, clientId, fetcher);
  if (managed instanceof Response) return managed;
  let body: any;
