...
function mapEquipment(row:SnapshotGeneralRow,prefix:'weapon'|'mount'):SnapshotEquipmentValue|null {
...
export async function createDeckSnapshot(env:Pick<DataEnv,'DB'>,userId:string,accountId:string,deckId:string,input:CreateSnapshotInput):Promise<CreateDeckSnapshotResult> {
  ...
  const generalRows=await env.DB.prepare(`SELECT gs.position,gs.general_id,g.name AS general_name,
  ...
  const tacticRows=await env.DB.prepare(`SELECT ts.general_position,ts.slot,ts.tactic_id,t.name AS tactic_name,
  ...
  const payload:SnapshotPayload={
    ...
    generals:generalRows.results.map((row)=>{
      ...
      return {
        ...
        weapon:mapEquipment(row,'weapon'),
        mount:mapEquipment(row,'mount'),
