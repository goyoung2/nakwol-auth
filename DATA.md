...
최초 배포는 `ops/data-bootstrap.flag` → exact `nakwol-data` D1 확인/생성 → migration → Worker deploy. 이후는 `ops/data-deploy.flag` → existing D1 필수 → migration → deploy이며 일반 배포는 D1을 새로 만들지 않습니다.

## Next

1. Registry importer와 실제 장수/전법 ID 적재
